<template>
  <div class="home">
    ...
    <!-- Formulario de inicio de sesión y transacción -->
...
export default {
  ...
  data() {
    return {
      ...
      apiClient: null, // Agregar el cliente API como propiedad de datos
    };
  },
  ...
  methods: {
    ...
    guardarTransaccion() {
      ...
      this.apiClient // Utiliza el cliente API ya creado
        .post('transactions', nuevaTransaccion)
        .then((response) => {
        ...
        text: 'No se pudo realizo la transacción.',
        icon: 'error',
        confirmButtonText: 'Aceptar',
    });
        });
    ...
    verificarCantidadCriptomonedas() {
      ...
      const url = `transactions?q={"user_id":"${idDeUsuario}","crypto_code":"${codigoCriptomoneda}"}`;
